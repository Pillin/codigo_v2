...
void Terrain::setMapPixel(int ancho_pix, int largo_pix){
    ...
    std::cout << ancho_pix << "  lala  " << largo_pix << std::endl;
...
std::vector<float> Terrain::getAngleValuePoints(){
    std::vector<float> angle_value_point;
    for( runnel::Triangle* tri: struct_triangle){
        angle_value_point.push_back(std::min(tri->minimun_value_angle_point[0],1.0f));
        angle_value_point.push_back(std::min(tri->minimun_value_angle_point[1],1.0f));
        angle_value_point.push_back(std::min(tri->minimun_value_angle_point[2],1.0f));
    }
    return angle_value_point;
}
