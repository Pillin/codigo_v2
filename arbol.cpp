...
//    ();
// for(arbol* h: hijos){
//     edges.addAll(h->getArbolEdges())
//     edges.add(p.coords);
//             edges.add(h->pto->coord);
// }
...
void arbol::getColorEdges(std::vector<glm::vec3>& color_edges){
    for (arbol* h: hijos){
   //   std::cout << "horton "  << h->number_strahler_horton << " ";
...
void arbol::getNumberStrahlerHorton(){
    ...
    for(arbol*h : hijos){
        if(h->number_strahler_horton == NO_NUMBER_STRAHLER_HORTON){
            ...
            //this->numbers_strahler_horton.push_back(h->number_strahler_horton);
        }
        ordenes[h->number_strahler_horton] +=1;
    } //ya todos tendran numero de strahler_horton
    int mayor_ord = 0;
